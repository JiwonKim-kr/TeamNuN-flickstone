...
func test_catalog(catalog: ContentCatalog) -> void:
	...
	check("P4-2-CATALOG-V11-COUNTS", status.is_ok() and catalog.catalog_schema_version() == 11 and catalog.registry_entry_count() == 30 and catalog.act_count() == 1 and catalog.encounter_count() == 4 and catalog.enemy_count() == 5 and catalog.relic_count() == 1 and catalog.consumable_count() == 1 and catalog.shop_count() == 1 and catalog.event_count() == 1)
