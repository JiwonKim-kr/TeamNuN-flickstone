...
func _init() -> void:
	...
	if not status.is_ok():
		print("P1_BATCH_FAILURE:%d,%d" % [status.code(), status.operation()])
	...
	print("P1_BATCH_ROW:%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%s" % [report.result, report.turn_count, report.sim_tick_count, report.player_alive, report.enemy_alive, report.player_damage, report.enemy_damage, report.damage_destroyed, report.kill_boundary_destroyed, report.kill_zone_destroyed, report.forced_settle_count, report.terminal_hash])
