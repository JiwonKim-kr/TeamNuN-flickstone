...
const EXPECTED_FINGERPRINT := "16df0d24ed90733b2f5f8b3761fd37830154e550c74fc00adba3a9445fa07167"
...
func test_catalog(catalog: ContentCatalog) -> void:
	...
	check("P2-6-RUNTIME-PACKAGE-COUNTS-IDS", status.is_ok() and catalog.fingerprint_hex() == EXPECTED_FINGERPRINT and catalog.piece_count() == 3 and catalog.ability_count() == 1 and catalog.status_count() == 1 and catalog.synergy_count() == 2 and catalog.map_count() == 1 and catalog.enemy_count() == 5 and catalog.act_count() == 1 and catalog.encounter_count() == 4 and baduk.string_id() == "baduk_stone" and bottle.string_id() == "bottle_cap" and striker.string_id() == "graybox_striker")
	check("P2-6-PIECE-LEVEL1-VALUES", status.is_ok() and baduk.level_count() == 1 and bottle.level_count() == 1 and striker.level_count() == 1 and bottle_level.max_hp() == 90 and bottle_level.attack() == 24 and bottle_level.mass_raw() == 56 * FixMath.SCALE and striker_level.ability_ref_count() == 1)
