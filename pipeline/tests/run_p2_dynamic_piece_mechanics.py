#!/usr/bin/env python3
"""P2-4 dynamic piece mechanics, links, expiry, and snapshot v6 runner."""
...
TEST_SCRIPT = "res://pipeline/tests/p2_dynamic_piece_mechanics_test.gd"
...
def main(argv: list[str] | None = None) -> int:
    ...
    reference = subprocess.run(
        [sys.executable, str(project / "pipeline/tests/p2_dynamic_piece_reference.py")],
    ...
    if reference.returncode != 0:
        ...
        print("P2_DYNAMIC_PIECE_RUNNER_RESULT: FAIL")
        return 1

    if os.environ.get(SKIP_GODOT_ENV) == "1":
        print("[SKIP] GODOT-P2-4 explicitly skipped by verify --skip-godot")
        print("P2_DYNAMIC_PIECE_RUNNER_RESULT: PASS (Godot skipped explicitly)")
        return 0
    ...
    result = godot_test_support.run_godot(
        ...
        log_name="godot-p2-dynamic-piece.log", timeout=300,
    )
    output = result.stdout + result.stderr
    checks = [line for line in output.splitlines() if line.startswith(("[PASS]", "[FAIL]"))]
    for line in checks:
        print(line)
    if result.returncode == 0 and "P2_DYNAMIC_PIECE_MECHANICS_RESULT: PASS" in output and not any(line.startswith("[FAIL]") for line in checks):
        print(f"P2_DYNAMIC_PIECE_RUNNER_RESULT: PASS ({len(checks)} grouped checks)")
        return 0
