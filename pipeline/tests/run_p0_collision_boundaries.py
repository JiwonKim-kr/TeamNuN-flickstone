#!/usr/bin/env python3
"""P0-3 deterministic collision, wall, and kill-zone acceptance runner."""
...
TEST_SCRIPT = "res://pipeline/tests/p0_collision_boundaries_test.gd"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 72)
    print("P0-3 acceptance: circle collision / wall / kill boundaries")
    print(f"project: {project}")
    print("=" * 72)

    failures = _check_core_boundary(project)
    if failures:
        print("[FAIL] CORE-BOUNDARY engine-independent collision contract")
        for failure in failures:
            print(f"  {failure}")
        print("P0_COLLISION_BOUNDARIES_RUNNER_RESULT: FAIL")
        return 1
    print("[PASS] CORE-BOUNDARY engine-independent collision contract")

    if os.environ.get(SKIP_GODOT_ENV) == "1":
        print("[SKIP] GODOT-P0-3 explicitly skipped by verify --skip-godot")
        print("P0_COLLISION_BOUNDARIES_RUNNER_RESULT: PASS (Godot skipped explicitly)")
        return 0
    ...
    if godot is None:
        ...
        print("P0_COLLISION_BOUNDARIES_RUNNER_RESULT: ERROR")
        return 2
    ...
    try:
        imported = godot_test_support.run_godot(
            ...
            project,
            "--import",
            log_name="godot-p0-collision-boundaries-import.log",
        )
    ...
    if imported.returncode != 0:
        ...
        print((imported.stdout + imported.stderr).strip()[-5000:])
        return 2
    ...
    try:
        result = godot_test_support.run_godot(
            ...
            "--script",
            TEST_SCRIPT,
            log_name="godot-p0-collision-boundaries-test.log",
            timeout=180,
        )
    except subprocess.TimeoutExpired:
        print("[FAIL] GODOT-P0-3 timeout", file=sys.stderr)
        return 2
    ...
    for line in checks:
        print(f"  {line}")
    marker_ok = "P0_COLLISION_BOUNDARIES_RESULT: PASS" in output
    failed_checks = [line for line in checks if line.startswith("[FAIL]")]
    if result.returncode == 0 and marker_ok and not failed_checks:
        print(
            "P0_COLLISION_BOUNDARIES_RUNNER_RESULT: PASS "
            f"({len(checks)} grouped checks)"
        )
        return 0

    print(
        "P0_COLLISION_BOUNDARIES_RUNNER_RESULT: FAIL "
        f"(exit={result.returncode}, marker={marker_ok}, failed={len(failed_checks)})"
    )
    print(output.strip()[-8000:], file=sys.stderr)
    return 1
