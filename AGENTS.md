...
- P4-4 reward recruitment, gold, REST recovery/merge, revenge boon, catalog/fingerprint v8, and RunSnapshot v2 are approved and implemented. Runtime adds three reward profiles, a battle-scoped +25% player outgoing-damage revenge status, and L2/L3 stats for both roster pieces. Minimal Godot import/class registration passes; runners, regression, fingerprint KAT, and `verify --demo` are deliberately deferred to the single P4-6 accumulated validation run.
- P4-5 relic/consumable/shop/event framework and catalog/fingerprint v9 are approved and implemented. Runtime adds one development relic, consumable, shop, and event; RunState supports atomic fixed SHOP/EVENT choices, MAP_CHOICE life-flask use, sorted bounded inventory, and persistent victory-gold relic bonuses. RunSnapshot remains v2 and now validates non-empty inventory plus SHOP/EVENT pending state. The production Python parser and minimal Godot import/headless startup pass; dedicated runners, cross-KAT, regressions, and `verify --demo` remain deferred to P4-6.
- P4-6 run completion, single-slot save/continue, RunManager transaction, battle run mode, 640×1,024 run graybox, and quick four-run validation are approved for implementation. Demo completion uses four runs plus targeted milestones; 16-seed all-route and `verify --full` remain formal-release debt.
