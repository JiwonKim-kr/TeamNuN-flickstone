...
| 2 | [`p2_effect_resolution.md`](p2_effect_resolution.md) | **approved** · 2026-08-23 · 핵심 구현·통합 검증 완료, 수용 보강 진행 | 트리거→능력→효과의 고정 순서, 조건·대상·기초 효과 원자, rollback |
...
P2-2 `p2_effect_resolution.md`의 P2-E01~12 승인 범위에서 schema v2, typed 실행 정의, binding, 6개 원자, 원자적 resolver, BattleSnapshot v4와 첫 통합 검증을 완료했다. next-wave·한도 경계·1,000회 반복 수용 테스트를 보강하는 동안 개별 기물 수치와 복사·무적·상태 세부는 해당 후속 명세까지 미정으로 유지한다.
