...
`Q(n)=n×65,536`이다. 표의 좌표·속도는 읽기 쉬운 논리 단위로 표기하고 JSON 픽스처에는 대응하는 Q47.16 raw 정수를 기록한다. 별도 표기가 없으면 seed=`0123456789abcdef`, 반지름 Q(32), 무게 Q(64), 기물 마찰 Q(1), `destructible=true`, 반발 17/20, 기본 마찰 5/2초⁻¹, 정지 속도 Q(1/2)를 쓴다. 공통 WALL 판은 화면 기준 시계 방향 `(-512,-512)→(512,-512)→(512,512)→(-512,512)`다.
