...
| 구현 상태 | **구현·자동 검증·사람 플레이 검수 완료** · 2026-08-25 |
...
하이브리드 구현은 동일 P2 3대3 상태에서 실제 `command_for()` 1회가 **302~336ms**로 500ms 상한을 통과했다. 독립 Python 기준, Godot P3 narrow, P2-6 1,000회·16×2 terminal/snapshot 골든 이관, Godot 4.6.3 quick `verify --full`도 통과했다.

## 완료 기록

2026-08-25, 회색상자에 같은 시드·배치를 유지하며 `F1/F2/F3` 또는 `7/8/9`로 COMMON/ELITE/BOSS를 전환하는 검수 모드를 추가했다. 최종 P3 narrow에서 독립 기준·런타임 등급·후보 예산·결정론·세 등급 명령 유효성·500ms 성능 계약이 통과했고 로컬 headless 선택은 **132ms**였다. Godot import·메인 씬 smoke·manifest 및 640×1,024 실제 렌더도 통과했다.

사용자는 세 등급을 직접 비교한 뒤 전체 체감이 괜찮고 ELITE/BOSS가 꽤 어렵다고 승인했다. 기존 COMMON 행동 검수와 이번 등급 차이 검수를 합쳐 수용 기준 16을 충족했으며 P3를 완료 처리한다.
