...
| 구현 권한 | **없음** · R-01~10 승인 전 코어 구현 금지 |
| 구현 상태 | 미착수 |
...
이 문서는 구현 전 초안이다. `⬜ 미정` 수치를 확정값으로 가장하지 않으며, P0 사건 의미와 상태 해시 기준값을 바꾸는 R-01은 영향과 회귀 범위를 포함해 별도 승인받는다.
...
| ID | 결정 | 권장안 | 상태 |
|---|---|---|---|
| R-01 | P0 충돌 사실 확장과 회귀 비용 | 기존 고정 레이아웃의 `value_b`·`flags`에 질량 쌍·속력 대소 저장, P0 golden 재고정 | ⬜ 승인 대기 |
| R-02 | 피해 전투원 상태 소유권 | 별도 `BattleCombatant`, 물리 body·CTB participant와 body ID로 결합 | ⬜ 승인 대기 |
| R-03 | P1 최소 스탯 표현 | HP·공격력 정수 1~1,000,000, 크리티컬 0~10,000 bp | ⬜ 승인 대기 |
| R-04 | U-32 피해 계수 | 기준속도 1,024, 임계속도 64, 질량지수 1/2, 질량비 1/2~2 | ⬜ 승인 대기 |
| R-05 | U-33 크리티컬 | 2배, 비율 감소 뒤·아군 보정 전; P1-3 자동 판정은 0%만 허용 | ⬜ 승인 대기 |
| R-06 | 아군·중립 규칙 | 같은 비중립 진영만 1/2, 중립은 항상 전액 | ⬜ 승인 대기 |
| R-07 | 재충돌 쿨다운 | 12 물리 틱, 실제 피해가 성립한 쌍만 등록 | ⬜ 승인 대기 |
| R-08 | 동률·동일 스텝 정산 | 속력 완전 동률은 양방향 동시 적용, 사건 sequence 순 처리 | ⬜ 승인 대기 |
| R-09 | 체력 0 공통 파괴 | `CauseId.DAMAGE=3`과 `SimWorld.destroy_body()` append-only 확장 | ⬜ 승인 대기 |
| R-10 | 스냅샷·API·수용 규격 | BattleSnapshot v2, v1 읽기 호환, P1-3 narrow + P0 회귀 | ⬜ 승인 대기 |
...
R-01~10은 서로 맞물린 하나의 구현 기준선이다. 특히 다음 세 묶음은 사람 결정 없이는 진행하지 않는다.

1. **P0 호환성:** 충돌 payload 의미 확장과 golden hash 재고정 허용 여부
2. **체감 수치:** 기준속도 1,024 / 임계 64 / 질량 sqrt·1/2~2 / 아군 1/2 / cooldown 12틱 / crit 2배
3. **동시성:** 속력 동률 양방향 동시 피해와 같은 스텝에서 HP 0 기물의 후속 충돌 제외

승인 뒤에만 이 문서의 status를 `approved`로 바꾸고 P1-3 구현을 시작한다.
