...
- 기본 마찰 79/80에서 속도 2,048이 정지 임계 1/2 아래로 내려가는 이론 시간은 약 5.5초다. 8초 정상 예산은 여러 차례 반사·연쇄 충돌·지형 효과를 허용하면서 비정상 장기 턴을 구분할 여유가 있다 (PT-01 재승인).
...
C-01부터 C-09까지의 개별 결정과 이 명세 전체는 2026-08-20 사용자 승인으로 확정되었다. 같은 날 승인 범위의 구현을 완료했고, P1-1 narrow·P0-1~4 narrow·Godot 활성 `verify --full` 검증을 통과했다.
