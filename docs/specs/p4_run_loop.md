...
| P4-6 | [`p4_run_ui_save_completion.md`](p4_run_ui_save_completion.md) · 축약 Act 1 UI·저장/이어하기·자동 완주 | **상세 명세 승인 · 구현 대기** · 처음부터 보스까지 자동/사람 완주 |
...
| P4-R17 | 일상 및 데모 단계 종료 검증은 고정 2 seeds×대표 2 route의 **4런**과 표적 milestone을 사용한다. 16 seeds×모든 고정 route profile과 `verify --full`은 정식 릴리즈에서 수행하고, 1,000회는 RunState/graph/snapshot 순수 결정론에만 적용한다 | 데모 시간 제약을 반영하면서 정식 릴리즈 전수 검증을 명시적 부채로 유지 | ✅ 재승인 |
...
2026-08-25 사용자는 P4-F01~20과 P4-6 상세 명세를 승인했다. P4-F19에 따라 P4-R17의 데모 단계 종료 검증은 quick 4런과 표적 milestone로 줄이고, 16 seeds×모든 route profile 및 `verify --full`은 정식 릴리즈 검증으로 이연했다.
