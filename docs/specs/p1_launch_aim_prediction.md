...
| L-04 | 최대 발사 속도 | 기준 최대 1,536, 절대상한 2,048 | ✅ PT-03 재승인 |
...
| 상수 | 값 |
|---|---:|
| `BASE_MAX_LAUNCH_SPEED_RAW` | `1536 * FixMath.SCALE` |
...
- 기준 무게 64의 full power는 1,536, 무게 256은 768, 무게 16은 절대상한 2,048이 된다.

이유:

- 기준 속도 1,536은 640×1,024 플레이 전장에서 기본 마찰 3/2초⁻¹과 함께 첫 충돌 거리를 확보하며, 가벼운 기물에는 2,048 상한을 적용한다.
...
- 기준 mass64/full power 속도 1,536, mass16/full power 2,048, mass256/full power 768.
...
2026-08-23 P1-5 플레이테스트 결과로 L-04 기준 최대 속도를 1,536으로 재승인했다 (`p1_physics_tuning.md` PT-03).
...
사용자가 벽 반사 궤적의 비정상 꺾임을 확인하고 다음 보정을 승인했다.

- 충돌 판정은 승인된 본체 반지름과 권위 사본 시뮬레이션을 그대로 사용한다.
- `TrajectoryPoint.position`은 일반점뿐 아니라 벽·기물 충돌 사건점에서도 현재 행동자 중심 좌표만 사용한다.
- P0 사건의 접촉 좌표를 중심 궤적선에 섞지 않는다. 접촉점이 필요해지면 별도 marker 표시 데이터로 확장한다.
- 첫 기물 충돌, 두 번째 벽 반사, 정지·소멸·240틱 잘림 종료 계약은 유지한다.
