...
static func create(flags: int, friction_multiplier_raw: int, acceleration: FixVec2, offset: FixVec2, vertices: Array[FixVec2], duration_turns: int, status: ContentStatus) -> ZoneSpawnPayloadDefinition:
	...
	result._flags = flags; result._friction_multiplier_raw = friction_multiplier_raw; result._acceleration = acceleration.copy(); result._offset = offset.copy()
...
func copy() -> ZoneSpawnPayloadDefinition:
	...
	return create(_flags, _friction_multiplier_raw, _acceleration, _offset, _vertices, _duration_turns, status)
