...
static func create(id_ref: ContentIdRef, node_type_id: int, map_ref: ContentIdRef, enemy_refs: Array[ContentIdRef], reward_profile_numeric_id: int, status: ContentStatus) -> EncounterDefinition:
	var result := EncounterDefinition.new()
	if not status.is_ok(): return result
	if id_ref == null or not id_ref.is_initialized() or map_ref == null or not map_ref.is_initialized() or (node_type_id != RunNodeType.Value.NORMAL_BATTLE and node_type_id != RunNodeType.Value.ELITE_BATTLE and node_type_id != RunNodeType.Value.BOSS) or enemy_refs.size() < ContentLimits.ENCOUNTER_ENEMY_MIN_COUNT or enemy_refs.size() > ContentLimits.ENCOUNTER_ENEMY_MAX_COUNT or reward_profile_numeric_id <= 0 or reward_profile_numeric_id > ContentLimits.UINT32_MAX:
...
func copy() -> EncounterDefinition:
	...
	return create(_id_ref, _node_type_id, _map_ref, _enemy_refs, _reward_profile_numeric_id, status)
