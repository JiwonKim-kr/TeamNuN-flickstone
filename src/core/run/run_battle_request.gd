...
var _opening_status_numeric_id: int = 0
var _players: Array[RunBattlePlayerEntry] = []
var _enemies: Array[RunBattleEnemyEntry] = []
var _initialized: bool = false

static func create(fingerprint: PackedByteArray, request_sequence: int, act_numeric_id: int, node_id: int, node_type_id: int, encounter_numeric_id: int, map_numeric_id: int, seed_hi: int, seed_lo: int, opening_status_numeric_id: int, players: Array[RunBattlePlayerEntry], enemies: Array[RunBattleEnemyEntry], status: SimStatus) -> RunBattleRequest:
	var result := RunBattleRequest.new()
	if not status.is_ok(): return result
	if fingerprint.size() != 32 or request_sequence <= 0 or request_sequence > 0xFFFFFFFF or act_numeric_id <= 0 or act_numeric_id > 0xFFFFFFFF or node_id <= 0 or node_id > 0xFFFFFFFF or (node_type_id != RunNodeType.Value.NORMAL_BATTLE and node_type_id != RunNodeType.Value.ELITE_BATTLE and node_type_id != RunNodeType.Value.BOSS) or encounter_numeric_id <= 0 or encounter_numeric_id > 0xFFFFFFFF or map_numeric_id <= 0 or map_numeric_id > 0xFFFFFFFF or not UInt32Math.is_u32(seed_hi) or not UInt32Math.is_u32(seed_lo) or not UInt32Math.is_u32(opening_status_numeric_id) or players.size() < 3 or players.size() > ContentLimits.MAP_DEPLOY_MAX_COUNT or enemies.size() < ContentLimits.MAP_DEPLOY_MIN_COUNT or enemies.size() > ContentLimits.MAP_DEPLOY_MAX_COUNT:
	...
	result._battle_seed_hi = seed_hi; result._battle_seed_lo = seed_lo; result._opening_status_numeric_id = opening_status_numeric_id; result._initialized = true
	return result

func copy() -> RunBattleRequest:
	var status := SimStatus.new()
	return create(_content_fingerprint, _request_sequence, _act_numeric_id, _node_id, _node_type_id, _encounter_numeric_id, _map_numeric_id, _battle_seed_hi, _battle_seed_lo, _opening_status_numeric_id, _players, _enemies, status) if _initialized else RunBattleRequest.new()
...
func opening_status_numeric_id() -> int: return _opening_status_numeric_id
