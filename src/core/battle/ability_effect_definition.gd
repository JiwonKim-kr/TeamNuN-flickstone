...
enum Kind { INVALID = 0, DAMAGE = 1, HEAL = 2, KNOCKBACK = 3, PULL = 4, MODIFY_CT = 5, MODIFY_VELOCITY = 6, MODIFY_STAT = 7, TELEPORT = 8, SET_FLAG = 9 }
...
static func create(kind_id: int, selector: AbilitySelectorDefinition, value_a: int, value_b: int, operation_id: int, status: ContentStatus) -> AbilityEffectDefinition:
	...
	if kind_id < Kind.DAMAGE or kind_id > Kind.MODIFY_VELOCITY or selector == null or not selector.is_initialized() or operation_id != 0:
