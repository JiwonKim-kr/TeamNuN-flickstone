...
enum Kind { INVALID = 0, DAMAGE = 1, HEAL = 2, KNOCKBACK = 3, PULL = 4, MODIFY_CT = 5, MODIFY_VELOCITY = 6, MODIFY_STAT = 7, TELEPORT = 8, SET_FLAG = 9, APPLY_STATUS = 10, REMOVE_STATUS = 11, SPAWN_PIECE = 12, SPAWN_PROJECTILE = 13, TRANSFORM_PIECE = 14, ATTACH = 15, SPAWN_ZONE = 16 }
...
var _zone_payload: ZoneSpawnPayloadDefinition
var _initialized: bool = false

static func create(kind_id: int, selector: AbilitySelectorDefinition, value_a: int, value_b: int, operation_id: int, status: ContentStatus, spawn_payload: SpawnPayloadDefinition = null, transform_payload: TransformPayloadDefinition = null, attach_payload: AttachPayloadDefinition = null, zone_payload: ZoneSpawnPayloadDefinition = null) -> AbilityEffectDefinition:
	var result := AbilityEffectDefinition.new()
	if not status.is_ok(): return result
	if kind_id < Kind.DAMAGE or kind_id > Kind.SPAWN_ZONE or kind_id == Kind.TELEPORT or kind_id == Kind.SET_FLAG or selector == null or not selector.is_initialized():
	...
	var payload_count: int = (1 if spawn_payload != null else 0) + (1 if transform_payload != null else 0) + (1 if attach_payload != null else 0) + (1 if zone_payload != null else 0)
	...
	if kind_id == Kind.SPAWN_ZONE and (zone_payload == null or not zone_payload.is_initialized()):
	...
	result._zone_payload = null if zone_payload == null else zone_payload.copy()
	result._initialized = true
	return result

func copy() -> AbilityEffectDefinition:
	var status := ContentStatus.new(); return create(_kind_id, _selector, _value_a, _value_b, _operation_id, status, _spawn_payload, _transform_payload, _attach_payload, _zone_payload) if _initialized else AbilityEffectDefinition.new()
...
func zone_payload() -> ZoneSpawnPayloadDefinition: return ZoneSpawnPayloadDefinition.new() if _zone_payload == null else _zone_payload.copy()
